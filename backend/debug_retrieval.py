...
def initialize():
    global _knowledge_base, _word_map, _labse_interpreter
...
def test_query(query):
    ...
    print(f"Keywords: {important_words}")
    ...
    for item in _knowledge_base:
        ...
        score = cosine + boost
        results.append({'text': item['text'][:100] + "...", 'score': score, 'cosine': cosine, 'boost': boost})

    results.sort(key=lambda x: x['score'], reverse=True)
    
    print("\nTop 5 Results:")
    for i, res in enumerate(results[:5]):
        print(f"{i+1}. Score: {res['score']:.4f} (Cos: {res['cosine']:.4f} + Boost: {res['boost']:.2f})")
        print(f"   Text: {res['text']}")

if __name__ == "__main__":
    initialize()
    test_query("What is democracy?")
    test_query("What is the formula of force?")
